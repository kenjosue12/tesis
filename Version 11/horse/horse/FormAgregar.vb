...
Public Class FormAgregar
    ...
    Public Sub New(ByVal cadeDatosInfo As String, ByVal ins As String)
        ...
        If instruccion.Equals("a") Then '  si vamos a actualizar o editar un caballo
            ...
            'desbloqueamos los tab
            bloqueo_taps.Enabled = True
        End If
    ...
    Public Function procesaDatosCaballo(ByVal cadenaDatos)
        ...
        CBservicios.Text = cadenaDatosFunc(12)
    ...
    Public Function insertar(ByVal comando As String)
        ...
        If RadioButton4.Checked = True Then
            MsgBox("ingresar")
            Dim nombres = CBpersonas.Text
            Dim nombresSeparados = Split(nombres, " ")
            Dim nombre = nombresSeparados(0)
            Dim apellido = nombresSeparados(1)

            Dim cadenaSQLpropietario = "select identificacion from personas where nombres='" + nombre + "' and apellidos='" + apellido + "';"
            MsgBox(CBpersonas.Text)
        ...
        If RadioButton5.Checked = True Then
            MsgBox("es nuevo")
            If TBnombreProp.Text <> "nombre..." And TBapellidosProp.Text <> "apellidos..." And TBidentifi.Text <> "identificación..." Then
    ...
    Public Function carga_genealogia(id_gene As String, nivel As Integer, pariente As String)
        ...
        If nivel.Equals(1) Then
            ...
            End If
            If pariente.Equals("mama") Then 'es la madre entons debe cargar por el lado de la mapa
                TB_abuelom.Text = traer_pariente(id_gene, "papa")
                TB_abualam.Text = traer_pariente(id_gene, "mama")
                TB_bis_pat_m.Text = traer_pariente(id_gene, "abueloP")
                TB_bisa_pat_m.Text = traer_pariente(id_gene, "abuelaP")
                TB_bis_mat_m.Text = traer_pariente(id_gene, "abueloM")
                TB_bisa_mat_m.Text = traer_pariente(id_gene, "abuelaM")
            End If
        End If
        If nivel.Equals(2) Then
            If pariente.Equals("abuelop") Then 'es la madre entons debe cargar por el lado de la mapa
                TB_bis_pat_p.Text = traer_pariente(id_gene, "papa")
                TB_bisa_pat_p.Text = traer_pariente(id_gene, "mama")
            End If
            If pariente.Equals("abuelap") Then 'es la madre entons debe cargar por el lado de la mapa
                TB_bis_m_p.Text = traer_pariente(id_gene, "papa")
                TB_bisa_mat_p.Text = traer_pariente(id_gene, "mama")
            End If
            If pariente.Equals("abuelom") Then 'es la madre entons debe cargar por el lado de la mapa
                TB_bis_pat_m.Text = traer_pariente(id_gene, "papa")
                TB_bisa_pat_m.Text = traer_pariente(id_gene, "mama")
            End If
            If pariente.Equals("abuelam") Then 'es la madre entons debe cargar por el lado de la mapa
                TB_bis_mat_m.Text = traer_pariente(id_gene, "papa")
                TB_bisa_mat_m.Text = traer_pariente(id_gene, "mama")
            End If
        End If
    ...
    Private Sub TB_padre_caballo_TextChanged(sender As Object, e As EventArgs) Handles TB_padre_caballo.TextChanged
        ...
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 1, "papa")
            End If
        End If
    End Sub
    ...
    Private Sub TabControl1_Selecting(sender As Object, e As TabControlCancelEventArgs) Handles TabControl1.Selecting
        'controlamos que el usario solo seleccione la primer tab
        If bloqueo_taps.Enabled = False Then
            e.Cancel = True
            MessageBox.Show("Primero debe guardar los datos del Caballo Nuevo", "Pestaña Bloqueada", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
    End Sub

    Private Sub TB_madre_caballo_TextChanged(sender As Object, e As EventArgs) Handles TB_madre_caballo.TextChanged
        'evento que se dispara cuando existe un caballo con el nombre digitado en el campo de la mama
        Dim text As String = TB_padre_caballo.Text
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 1, "mama")
            End If
        End If
    End Sub

    Private Sub TB_abuelop_TextChanged(sender As Object, e As EventArgs) Handles TB_abuelop.TextChanged
        'evento que se dispara cuando existe un caballo con el nombre digitado en el campo del abuelo paterno
        Dim text As String = TB_padre_caballo.Text
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 2, "abuelop")
            End If
        End If
    End Sub

    Private Sub TB_abuelap_TextChanged(sender As Object, e As EventArgs) Handles TB_abuelap.TextChanged
        'evento que se dispara cuando existe un caballo con el nombre digitado en el campo la abuela paterna
        Dim text As String = TB_padre_caballo.Text
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 2, "abuelap")
            End If
        End If
    End Sub

    Private Sub TB_abuelom_TextChanged(sender As Object, e As EventArgs) Handles TB_abuelom.TextChanged
        'evento que se dispara cuando existe un caballo con el nombre digitado en el campo del abuelo materno
        Dim text As String = TB_padre_caballo.Text
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 2, "abuelom")
            End If
        End If
    End Sub

    Private Sub TB_abualam_TextChanged(sender As Object, e As EventArgs) Handles TB_abualam.TextChanged
        'evento que se dispara cuando existe un caballo con el nombre digitado en el campo de la abuela materna
        Dim text As String = TB_padre_caballo.Text
        Dim IDgenealo As String = buscar_nom_genealogia(text)
        If Not (IDgenealo.Equals("")) Then
            If MsgBox("Existe una Genealogia asociada a este Caballo, ¿desea cargarla ?", MsgBoxStyle.YesNo, "Cargar Genealogia") = MsgBoxResult.Yes Then
                carga_genealogia(IDgenealo, 2, "abuelam")
            End If
        End If
    End Sub
